...
class Chest {
    ...
    async add(target, options = {}) {
        ...
        if (urlInfo.protocol === 'http:' || urlInfo.protocol === 'https:') {
            ...
            name = options.name || options.type === CmdType.NPM ? require(path_1.join(target, 'package.json')).name : path_1.basename(target, path_1.extname(target));
        }
        else {
            // unknow type
            return;
            ...
            default:
                if (cmd.source) {
                    if (cmd.source.startsWith('http://') || cmd.source.startsWith('https://')) {
                        rimraf_1.default.sync(cmd.path);
                    }
                }
        }
